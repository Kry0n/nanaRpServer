...
RegisterCommand('emsblips', function()
	local source = PlayerId()
	local xPlayer = ESX.GetPlayerData()
	local job = xPlayer.job
	local jobname = xPlayer.job.name
	if job and jobname == 'ambulance' then
			 TriggerServerEvent('anubis:AddPlayerBlipEMS')
		end
end)

RegisterCommand('policeblips', function()
	local source = PlayerId()
	local xPlayer = ESX.GetPlayerData()
	local job = xPlayer.job
	local jobname = xPlayer.job.name
	if job and jobname == 'police' then
			 TriggerServerEvent('anubis:AddPlayerBlipPolice')
		end
end)