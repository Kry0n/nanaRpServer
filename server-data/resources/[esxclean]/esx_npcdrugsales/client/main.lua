          ...
          if IsPedInAnyVehicle(ped) == false then
            ...
            if pedType ~= 28 and IsPedAPlayer(ped) == false then
              ...
              if distance <= 2 and ped  ~= GetPlayerPed(-1) and ped ~= oldped then
                ...
                if has == true then
                  drawTxt(0.90, 1.40, 1.0,1.0,0.4, "按下 ~g~E ~w~來進行毒品交易...", 255, 255, 255, 255)
...
AddEventHandler('notifyc', function()
    ...
    if crossing ~= nil then
      ...
      TriggerServerEvent('esx_phone:send', "police", "一些陰險的藥頭在賣毒品在 " .. streetName .. " 還有 " .. crossing, true, {
        x = coords.x,
        y = coords.y,
        z = coords.z
      })
    else
      TriggerServerEvent('esx_phone:send', "police", "一些陰險的藥頭在賣毒品 " .. streetName, true, {
        x = coords.x,
        y = coords.y,
        z = coords.z
      })
