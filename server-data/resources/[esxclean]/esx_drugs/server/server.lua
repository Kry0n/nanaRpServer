		...
		if PlayersSelling[source] == true and Drug[source] == drug then
			...
			if item == 0 then
				...
				if CopsConnected <= 1 then
					--xPlayer.addAccountMoney('bank', math.random(v.Zones.Dealer.sellMin, v.Zones.Dealer.sellMax))
					xPlayer.addMoney(math.random(v.Zones.Dealer.sellMin, v.Zones.Dealer.sellMax))
				else
					--xPlayer.addAccountMoney('bank', math.random( v.Zones.Dealer.sellMin * CopsConnected, v.Zones.Dealer.sellMax * CopsConnected))
					xPlayer.addMoney(math.random( v.Zones.Dealer.sellMin * CopsConnected, v.Zones.Dealer.sellMax * CopsConnected))
				end
