		...
		if not IsPedInAnyVehicle(GetPed(), false) then
			if not IsPedSwimming(GetPed()) then
				if IsEntityInWater(GetPed()) then
					...
					IsFishing = true
					if ShowChatMSG then ESX.ShowNotification("您丟出了誘餌，請等待魚上鉤 ...") end
					RunCodeOnly1Time = true
					BarAnimation = 0

				else
					ESX.ShowNotification('不可能動作，你必須在水中')
				end
			else
				ESX.ShowNotification('不可能動作')
			end
		else
			ESX.ShowNotification('不可能動作')
		end
	else
		ESX.ShowNotification("你一定是魚販子.")
	end
end)
