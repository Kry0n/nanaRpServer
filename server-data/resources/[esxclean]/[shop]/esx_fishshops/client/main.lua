...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('esx_fishshops:requestDBItems', function(ShopItems)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				TriggerServerEvent('esx_fishshops:buyItem', data.current.item, data.current.value, zone)
			end
...
AddEventHandler('esx_fishshops:hasEnteredMarker', function(zone)
	currentAction     = 'shop_menu'
	currentActionMsg  = _U('press_menu')
	currentActionData = {zone = zone}
end)

AddEventHandler('esx_fishshops:hasExitedMarker', function(zone)
	currentAction = nil
	ESX.UI.Menu.CloseAll()
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not hasAlreadyEnteredMarker then
			...
			TriggerEvent('esx_fishshops:hasEnteredMarker', currentZone)
		end

		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			TriggerEvent('esx_fishshops:hasExitedMarker', lastZone)
		end
